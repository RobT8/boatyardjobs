...
export default function AdvertiserAccountStep({ next, autherror, sent, defaultMode }: Props) {
  ...
  return (
    <div>
      ...
      {mode === "login" && !forgot && (
        <form action="/api/ads/login" method="post" className="space-y-3">
      ...
      {mode === "login" && forgot && (
        <form action="/api/ads/magic-link" method="post" className="space-y-3">
      ...
      {mode === "register" && (
        <form action="/api/ads/register" method="post" className="space-y-3">
          <input type="hidden" name="next" value={next} />
